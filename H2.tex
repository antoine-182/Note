...
\subsection{Prescription de Glover et Janev (sans les nouveaux taux de collisions)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Comparaison modèles - Janev/Glover}

On utilise un modèle type Barre d'Orion (OrbarBestMod) qui est un modèle isobare et cherchons à voir les changements sur la PDR. On voit que sur la figure \ref{fig:H2:JanevGlover:model} que la transition HH2 s'effectue au même $A_\mathrm{V}$ et que la température au bord du nuage est légèrement plus chaude ($+200K$) avec la prescription de Glover. On observe de plus une bosse dans la température au début du nuage moléculaire. Pourquoi ? \newline 


Les raies d'émissions de $\mathrm{H}_2$ et $\mathrm{CO}$ sont plus intenses en utilisant la prescription de Glover (figure \ref{fig:H2:JanevGlover:raiesCOH2}). Les profils des traceurs à travers le nuage révèle que l'on parvient à former plus de $\mathrm{CO}$ après le front moléculaire. Elle s'accompagne d'une hausse de température local. Elle est provoquée par l'augmentation local du chauffage par réactions chimiques. On sait que les dissociations collisionnelles sont les réactions endothermiques les plus efficaces pour refroidir le gaz. Les taux de réactions calculés par Glover les tuent ce qui permet à la recombinaison électronique du $\mathrm{CH}_3^+$ de chauffer le gaz. On voit sur la figure \ref{fig:H2:JanevGlover:heatingr} les taux de chauffage relatifs à travers le nuage : on voit clairement que les réactions chimiques commencent à chauffer le gaz au début du nuage moléculaire.

\textit{A quoi est du le léger décrochage sur le profil de H dans Glover ?}
\textit{j'ai l'impression que l'on a pas ca dans les modèles à densités constantes}

\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/janev/profil_H.png}
        \caption{Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/glover/profil_H.png}
        \caption{Glover}
    \end{subfigure}
    \caption{Profil $\mathrm{H}$, $\mathrm{H}_2$ et température}
    \label{fig:H2:JanevGlover:model}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/I_comp_CO.pdf}
        \caption{CO - Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/I_comp_H2.pdf}
        \caption{H2 - Glover}
    \end{subfigure}
    \label{fig:H2:JanevGlover:Gmax}

    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/nT_comp_CO.pdf}
        \caption{$n(\mathrm{CO})$ - Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/nT_comp_H2.pdf}
        \caption{$n(\mathrm{H}_2)$ - Glover}
    \end{subfigure}
    
    \caption{}
    
    \label{fig:H2:JanevGlover:raiesCOH2}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/janev/heating_r.png}
        \caption{Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/orbar_janevVSglover/glover/heating_r.png}
        \caption{Glover}
    \end{subfigure}
    \caption{Taux de chauffages relatifs en fonction de la profondeur du nuage.}
    \label{fig:H2:JanevGlover:heatingr}
\end{figure}


\subsubsection{Bord Atomique - Janev/Glover}

On lancé des grilles, à densités constantes, et avons chercher à voir les différences qu'engendrent la nouvelle prescription.

En bord de région atomique, le chauffage par $\mathrm{H}_2$ prédomine sur les mêmes types de PDR (figure \ref{fig:H2:JanevGlover:Gmax}). Les processus de refroidissement sont différents (figure \ref{fig:H2:JanevGlover:Lmax}) : les réactions chimiques ne refroidissent plus les bords atomiques des PDR denses et faiblement illuminées. En effet, le calcul de Glover ne favorise plus la dissociation collisionelle de $\mathrm{H}_2$ qui est endothermique. La température de la partie atomique du nuage n'est que très peu affecté sinon qu'elle est plus chaude d'environ $200K$.

\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_janev/mapGmax.pdf}
        \caption{Chauffage - Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_glovernoBossion/mapGmax.pdf}
    ...
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_janev/mapLmax.pdf}
        \caption{Refroidissement - Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_glovernoBossion/mapLmax.pdf}
...
\begin{figure}[!h]
    ...
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_glovernoBossion/mapTba.pdf}
        \caption{Glover}
    \end{subfigure}
    \caption{Température au bord atomique de la PDR}
    \label{fig:H2:JanevGlover:Tba}
\end{figure}

On a tracé sur la figure \ref{fig:H2:JanevGlover:H2} le rapport $\Gamma_{\mathrm{H}_2}/\Gamma_{\mathrm{tot}}$ au bord atomique du nuage. 

\textit{Est que le chauffage $\Gamma_{\mathrm{H}_2}$ perd de son efficacité car les bords atomiques sont plus chauds?}

\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_janev/mapG_H2.pdf}
        \caption{Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_glovernoBossion/mapG_H2.pdf}
        \caption{Glover}
    \end{subfigure}
    \caption{Rapport du chauffage par $\mathrm{H}_2$ sur le chauffage total en bord de région atomique. Le trait rouge marque la limite où le chauffage par $\mathrm{H}_2$ est prédominant}
    \label{fig:H2:JanevGlover:H2}
\end{figure}
...
On s'intéresse maintenant aux effets sur la transition $\mathrm{H}/\mathrm{H}_2$. En utilisant la formule de Glover on dépasse le plateau de $600 K$ qu'avait Franck. On atteint des températures allant jusqu'au millier de Kelvin (figure \ref{fig:H2:JanevGlover:THH2} - il faut refaire les cartes, THH2 est trompeur).

\textit{Il faut comprendre pourquoi on dépasse ce plateau ? Pour cela on peut racer les .res aux niveaux de la transition afin de comprendre si des refroidisseurs manquent or not ?}

\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_janev/HH2_T_Franck.pdf}
        \caption{Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
...
Sur la figure \ref{fig:H2:JanevGlover:AVHH2}, on voit que la taille de la zone atomique est la même (ce qui attriste Franck car le modèle n'arrive pas à retrouver les dimensions observées).

\begin{figure}[!h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth} % "0.45" donne ici la largeur de l'image
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_janev/HH2_AV_Franck.pdf}
        \caption{Janev}
    \end{subfigure}
    ~ 
    \begin{subfigure}[t]{0.45\textwidth}
        \centering \includegraphics[trim = {0 0 0 0 },clip,width=1\textwidth]{figure/H2/grid_glovernoBossion/HH2_AV_Franck.pdf}
        \caption{Glover}
    \end{subfigure}
    \caption{Température transition HH2}
...
\begin{itemize}
    ...
    \item Si l'on se place à grand $\chi$ (b) le chauffage par $\mathrm{H}_2$ commence à prédominer en bord de nuage et on voit qu'il efface l'augmentation de température en bord de nuage moléculaire à mesure que le nuage devient dense. On voit un phénomène similaire en comparant (a) et (c) : la recombinaison des grains s'intensifie avec le champs de rayonnement.
