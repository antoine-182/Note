...
%     \begin{tabular}{|l|c|c|}
%     \hline
%      X & $n(\mathrm{X})/n(\mathrm{H})$ & $\mathrm{PI}\ (\mathrm{eV})$   \\
%      \hline
%      H    &  $1$          &     13.6   \\
%      He   &  $0.1$        &    24.6      \\
%      O    &  $3.2\,10^{-4}$ &    13.6    \\
...
% Néanmoins, la formule précédente ne prend pas en compte les PAH qui sont efficace pour chauffer le gaz. Nous avons comparé une autre expressions de $ \Gamma_{\mathrm{pe}}$ (\cite{Wolfire_2003} Eq 20, \cite{BakesTielens1994}) qui est de la forme
% \begin{equation}
...
% \end{equation}
% avec $\phi_\mathrm{PAH}$ un paramètre représentant l'efficacité des collisions ($0<\phi_\mathrm{PAH}<1$). Sur la figure \ref{fig:Cl:pePAH} sont comparés les deux expressions de $ \Gamma_{\mathrm{pe}}$ avec le calcul du code PDR ne prenant pas en compte les PAH. du code:  paramètres du code PDR $r_\mathrm{min} = 10^7\,\mathrm{nm}$. 
% \begin{figure}[!h]
%     \centering
%     \begin{subfigure}[t]{0.49\textwidth} % "0.49" donne ici la largeur de l'image
